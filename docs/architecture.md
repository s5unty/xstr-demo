...
## 模块分层
1. 渲染层 `src/render/*`：Three.js 场景、反馈动画、3D 键盘与视角控制。
...
## 关键数据流
`KeyboardEvent | 3DKeyClick -> QuickCodeIme(raw/candidates) -> committedText -> GameEngine(compare/metrics) -> ThreeScene(feedback/key-anim) + InputOverlay(render)`
